let processor = {
	doLoad: function() {
		...
		this.layer.push({ show: true, type: 'convolute', weights: [ -1, -1,
																	 1,  1], input: this.layer[0] });
		this.layer.push({ show: false, type: 'vector', output: this.layer[0], input: [ this.layer[1], this.layer[2], this.layer[3] ] });
...
let filter = {
	...
	vector : function(input) {
		...
		let x, y, d, vx, vy, xx, yy, ix, iy, sqrt, mmmm, i=0, ctx = this.output.contx;
		ctx.beginPath();
		for (y=0; y<h; y++) {
			for (x=0; x<w; x++, i++) {
				d = inpD[i];
				if (-tres < d && d < tres) continue;
				ix = inpX[i];
				iy = inpY[i];
				sqrt = Math.sqrt(ix * ix + iy * iy);
				vx = d * ix / sqrt;
				vy = d * iy / sqrt;
				xx = x/scale;
				yy = y/scale;
				ctx.moveTo(0.5 + xx, 0.5 + yy);
				ctx.lineTo((x + f*vx)/scale, (y + f*vy)/scale);
				ctx.rect(xx,yy,1,1);
			}
		}
